...
theorem FiniteDimensional.finrank_mul_finrank'' (F K L : Type _) [Field F] [Field K] 
...
theorem sup_multiset_prod_eq_top {R : Type _} [CommSemiring R] {I : Ideal R} 
...
theorem ramificationIdx_tower [IsDomain S] [IsDedekindDomain S] [IsDomain T] [IsDedekindDomain T] 
    {f : R →+* S} {g : S →+* T} 
...
theorem ramificationIdx_algebra_tower [IsDomain S] [IsDedekindDomain S] [IsDomain T] 
    [IsDedekindDomain T] {p : Ideal R} {P : Ideal S} {Q : Ideal T} [hpm : IsPrime P] 
    [hqm : IsPrime Q] (hf0 : map (algebraMap R S) p ≠ ⊥) (hg0 : map (algebraMap S T) P ≠ ⊥) 
...
variable {R S : Type _} [CommRing R] [CommRing S] (f : R →+* S)
...
-- Proposition 8.4
theorem OnlyFinitelyManyPrimeidealsRamified (R S : Type _) [CommRing R] [IsDomain R] 
  [IsDedekindDomain R] [CommRing S] [IsDomain S] [Algebra R S] (K L : Type _) [Field K] [Field L] 
  [Algebra K L] [FiniteDimensional K L] [IsSeparable K L] [Algebra R K] [IsFractionRing R K] 
  [Algebra S L] [IsFractionRing S L] [Algebra R L] [IsScalarTower R S L] [IsScalarTower R K L] 
...
instance {p : ℕ} [hpp : Fact (Nat.Prime p)] : IsMaximal (Submodule.span ℤ {(p : ℤ)}) :=
  PrincipalIdealRing.isMaximal_of_irreducible <|
    PrincipalIdealRing.irreducible_iff_prime.mpr (Nat.prime_iff_prime_int.mp (Fact.elim hpp))
